...
export default function HomePage({
  ...
  const {signerAddress, totalOwnerDusty, homeLoading} = useContext(ConnectionContext);
  return (
        ...
        <div className="homepage">
          {connected &&
            <div className="home-row">
              <div className="dashboard-item user-box" style={{ background: "linear-gradient(135deg, #8a00ff, #450a8f" }}>
                <h2>Your Total NFTs&nbsp;({signerAddress.slice(0, 4) + "..." + signerAddress.slice(39, 42)})</h2>
                <p>
                  {connected ?
          ...
          <div className="home-row">
            ...
            <div className="dashboard-item">
              ...
              <p>{connected ? (!homeLoading ? new Intl.NumberFormat().format(ethers.utils.formatEther((totalOwnerDusty || 0.0).toString())) : <Skeleton width={120} sx={{ bgcolor: '#ffffff20' }} height={50} style={{ marginLeft: "auto", backgroundColor: "ffffff3d" }} />) : <span>N/A</span>}</p>
              <svg width="80" height="80" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <p>
    {connected ? (
      !homeLoading ? (
        (() => {
          const totalSupplyValue = totalSupply ? parseFloat(totalSupply) : 0;
          const totalDustyValue = totalDusty ? parseFloat(ethers.utils.formatEther(totalDusty)) : 0;

          const formattedValue = new Intl.NumberFormat().format(
            10000000 - totalSupplyValue + totalDustyValue
          );
